import { RemoteDatabase, RemoteDatabaseInitOptions } from "../core/interfaces.js";
import { RemoteSnapshot } from "../core/types.js";
import { RemoteDatabaseError, OutOfDateError, NetworkError } from "../core/errors.js";
...
export abstract class BaseRemoteDatabase implements RemoteDatabase {
  ...
  protected wrapError(error: unknown, context: string): RemoteDatabaseError {
    ...
    if (error instanceof Error) {
      ...
      if (message.includes("unauthorized") || message.includes("forbidden") || message.includes("auth")) {
        return new NetworkError(`${context}: Authentication failed - ${error.message}`, error);
      }

      if (message.includes("not found") || message.includes("404")) {
        return new RemoteDatabaseError(`${context}: Resource not found - ${error.message}`, "NOT_FOUND", error, this.options.endpointId);
      }
    }

    return new RemoteDatabaseError(`${context}: ${error instanceof Error ? error.message : String(error)}`, "UNKNOWN_ERROR", error instanceof Error ? error : undefined, this.options.endpointId);
  }
  ...
  protected validateSnapshot(snapshot: Omit<RemoteSnapshot, "commitId">): void {
    if (!snapshot.schemas || !Array.isArray(snapshot.schemas)) {
      throw new RemoteDatabaseError("Invalid snapshot: schemas must be an array", "VALIDATION_ERROR", undefined, this.options.endpointId);
    }

    if (!snapshot.records || !Array.isArray(snapshot.records)) {
      throw new RemoteDatabaseError("Invalid snapshot: records must be an array", "VALIDATION_ERROR", undefined, this.options.endpointId);
    }

    // Validate schemas
    for (const schema of snapshot.schemas) {
      if (!schema.name || typeof schema.name !== "string") {
        throw new RemoteDatabaseError("Invalid schema: name is required and must be a string", "VALIDATION_ERROR", undefined, this.options.endpointId);
      }

      if (!schema.fields || !Array.isArray(schema.fields)) {
        throw new RemoteDatabaseError("Invalid schema: fields must be an array", "VALIDATION_ERROR", undefined, this.options.endpointId);
      }
    }
    ...
    for (const record of snapshot.records) {
      if (!record.id || typeof record.id !== "string") {
        throw new RemoteDatabaseError("Invalid record: id is required and must be a string", "VALIDATION_ERROR", undefined, this.options.endpointId);
      }

      if (!record.schema || typeof record.schema !== "string") {
        throw new RemoteDatabaseError("Invalid record: schema is required and must be a string", "VALIDATION_ERROR", undefined, this.options.endpointId);
      }

      if (!record.data || typeof record.data !== "object") {
        throw new RemoteDatabaseError("Invalid record: data is required and must be an object", "VALIDATION_ERROR", undefined, this.options.endpointId);
      }
    }
  }
   ...
   */
  protected commitIdsMatch(id1: string | undefined, id2: string | undefined): boolean {
   ...
   */
  protected createOutOfDateError(baseCommitId: string, actualCommitId: string): OutOfDateError {
