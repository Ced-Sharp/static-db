import { SchemaDef, EntityRecord, FieldDef, isSchemaDef, isEntityRecord } from "../core/types.js";
import { ValidationError, SchemaValidationError } from "../core/errors.js";
...
export class CMSValidator {
  ...
  validateSchema(schema: unknown): ValidationResult {
    ...
    if (!isSchemaDef(schema)) {
      errors.push(new ValidationError("Invalid schema structure", undefined, schema));
  ...
  validateField(field: unknown): ValidationResult {
    ...
    if (!field || typeof field !== "object") {
      errors.push(new ValidationError("Field must be an object", undefined, field));
      return { isValid: false, errors, warnings };
    }
    ...
    if (!fieldObj.name || typeof fieldObj.name !== "string") {
      errors.push(new ValidationError("Field name is required and must be a string", "name", field));
    }

    if (!fieldObj.type || typeof fieldObj.type !== "string") {
      errors.push(new ValidationError("Field type is required and must be a string", "type", field));
    }
    ...
    if (fieldObj.name && typeof fieldObj.name === "string") {
      ...
      if (fieldObj.name.startsWith("_")) {
        warnings.push(`Field name '${fieldObj.name}' starts with underscore (reserved for system fields)`);
      }
    }
    ...
    if (fieldObj.type === "relation" && fieldObj.relationSchema) {
      if (!this.schemas.has(fieldObj.relationSchema)) {
        warnings.push(`Relation field '${fieldObj.name}' references unknown schema '${fieldObj.relationSchema}'`);
      }
    }
    ...
    if (fieldObj.type === "media" && fieldObj.acceptedTypes) {
      if (!Array.isArray(fieldObj.acceptedTypes)) {
        errors.push(new ValidationError("acceptedTypes must be an array", "acceptedTypes", field));
      }
    }
  ...
  validateRecord(record: unknown): ValidationResult {
    ...
    if (!isEntityRecord(record)) {
      errors.push(new ValidationError("Invalid record structure", undefined, record));
      return { isValid: false, errors, warnings };
    }
   ...
   */
  validateRecordData(record: EntityRecord, schema: SchemaDef): ValidationResult {
    const errors: ValidationError[] = [];
    const warnings: string[] = [];
    ...
    // Check required fields
    for (const field of schema.fields) {
      const hasValue = data[field.name] !== undefined && data[field.name] !== null;
      ...
      if (hasValue) {
        const fieldResult = this.validateFieldValue(data[field.name], field, schema);
        errors.push(...fieldResult.errors);
        warnings.push(...fieldResult.warnings);
      }
    }
    ...
    if (!record.createdAt || !this.isValidISODate(record.createdAt)) {
      errors.push(new ValidationError("Invalid createdAt timestamp", "createdAt", record.createdAt));
    }

    if (!record.updatedAt || !this.isValidISODate(record.updatedAt)) {
      errors.push(new ValidationError("Invalid updatedAt timestamp", "updatedAt", record.updatedAt));
    }
   ...
   */
  private validateFieldValue(value: unknown, field: FieldDef, schema: SchemaDef): ValidationResult {
    const errors: ValidationError[] = [];
    const warnings: string[] = [];
    ...
    try {
      switch (field.type) {
        ...
        default:
          ...
          warnings.push(`Unknown field type '${field.type}' for field '${field.name}'`);
      }
    } catch (error) {
      errors.push(new ValidationError(
   ...
   */
  private validateConstraints(value: unknown, constraints: Record<string, unknown>, fieldName: string): ValidationResult {
    const errors: ValidationError[] = [];
    const warnings: string[] = [];
    ...
    for (const [constraintName, constraintValue] of Object.entries(constraints)) {
      try {
        switch (constraintName) {
          ...
          case "pattern":
            if (typeof value === "string" && !new RegExp(constraintValue as string).test(value)) {
              errors.push(new ValidationError(
          ...
          default:
            warnings.push(`Unknown constraint '${constraintName}' for field '${fieldName}'`);
        }
      } catch (error) {
        errors.push(new ValidationError(
