...
export class IndexedDBLocalDatabase implements LocalDatabase {
  private readonly options: Required<
    Omit<IndexedDBLocalDatabaseOptions, "debug">
  > & { debug: boolean };
  ...
  async save(
    ...
    try {
      const transaction = (this.db as IDBDatabase).transaction(
        [this.options.storeName, this.options.metaStoreName],
        "readwrite",
      );
  ...
  private async openDatabase(): Promise<IDBDatabase> {
    return new Promise((resolve, reject) => {
      ...
      request.onblocked = () => {
        this.debugLog(
          "Database open blocked - waiting for other connections to close",
        );
      };
  ...
  private createObjectStores(db: IDBDatabase): void {
    ...
    if (!db.objectStoreNames.contains(this.options.storeName)) {
      const snapshotStore = db.createObjectStore(this.options.storeName, {
        keyPath: "id",
      });
    ...
    if (!db.objectStoreNames.contains(this.options.metaStoreName)) {
      db.createObjectStore(this.options.metaStoreName, {
        keyPath: "key",
      });
    }
  }
  ...
  private async loadSnapshot(): Promise<RemoteSnapshot | null> {
    return new Promise((resolve, reject) => {
          ...
          try {
            ...
            reject(
              new DataCorruptionError(
                `Invalid snapshot format: ${error instanceof Error ? error.message : String(error)}`,
              ),
            );
          }
        } else {
      ...
      request.onerror = () => {
        reject(
          new Error(
            `Failed to load snapshot: ${request.error?.message || "Unknown error"}`,
          ),
        );
      };
    });
  ...
  private async saveSnapshotInTransaction(
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () =>
        reject(
          new Error(
            `Failed to save snapshot: ${request.error?.message || "Unknown error"}`,
          ),
        );
    });
  }

  private async saveMetadataInTransaction(
    transaction: IDBTransaction,
    options: LocalSaveOptions,
  ): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () =>
        reject(
          new Error(
            `Failed to save metadata: ${request.error?.message || "Unknown error"}`,
          ),
        );
    });
  }
